...
def extract_plan_json(text: str) -> dict | None:
    ...
    if not isinstance(obj, dict) or not isinstance(obj.get("tasks"), list):
        raise PlanError("计划不是合法的 JSON,或缺少 tasks 数组")
    return obj
...
def build_plan(obj: dict, members: list[dict], max_tasks: int = 8, known_tools: set[str] | None = None) -> Plan:
    ...
    if not isinstance(obj.get("tasks"), list):
        raise PlanError("tasks 不是列表")
    raw_tasks = obj["tasks"][:max_tasks]
    if not raw_tasks:
        raise PlanError("tasks 是空的")
    tasks: list[PlanTask] = []
    used: set[str] = set()
    remap: dict[str, str] = {}
    for i, rt in enumerate(raw_tasks, 1):
        if not isinstance(rt, dict):
            raise PlanError(f"第 {i} 个任务不是对象")
        owner = _match_member(rt.get("owner", ""), members)
        if not owner:
            raise PlanError(f"任务「{rt.get('title') or i}」的负责人「{rt.get('owner')}」不是群成员")
        ...
        if not instruction:
            raise PlanError(f"任务 {tid} 缺少 instruction")
...
def _toposort(tasks: list[PlanTask]) -> list[PlanTask]:
    ...
    while remaining:
        ...
        if ready is None:
            raise PlanError("任务之间存在循环依赖:" + "、".join(t.id for t in remaining))
...
def planning_instruction(max_tasks: int, mode: str, past_actions: str = "") -> str:
    force = ("这次必须分工:请直接输出计划。" if mode == "on"
             else "先判断:如果这件事你一个人就能答好(闲聊、简单问答、单一步骤),就直接回答,不要输出 <plan>。")
    return (
        "【分工模式】你是群主,现在要先安排团队。\n" + force + "\n"
...
def assignments_text(plan: Plan, me: PlanTask) -> str:
    lines = []
    for t in plan.tasks:
        mark = "  ← 你" if t.id == me.id else ""
        dep = f"(承接 {'、'.join(t.needs)})" if t.needs else ""
...
def task_prompt(plan: Plan, task: PlanTask, outputs: dict[str, str], index: int) -> str:
    ...
    for nid in task.needs:
        ...
        if up and outputs.get(nid):
            ups.append(f"[{up.id} · {up.owner} 的成果:{up.title}]\n{_clip(outputs[nid], 3500)}")
        elif up:
            ups.append(f"[{up.id} · {up.owner}:{up.title}] 该任务没有产出(状态:{up.status}),请基于已有信息尽力完成,并说明缺了什么。")
    parts = [
        f"【分工任务 {index}/{len(plan.tasks)}】{task.title}",
        f"总目标:{plan.goal or '(见用户需求)'}",
    ]
    if plan.conventions:
        parts.append(f"全组统一约定(必须遵守,不得各说各话):\n{plan.conventions}")
    parts.append(f"全组分工:\n{assignments_text(plan, task)}")
    parts.append(f"你的任务:{task.instruction}")
    if task.deliverable:
        parts.append(f"交付物:{task.deliverable}")
    if task.strengths:
        parts.append(f"群主认为这项任务需要你发挥:{'、'.join(task.strengths)}。")
    if task.tools:
        parts.append(f"建议优先使用的工具:{'、'.join(task.tools)}(需要时先调用,别凭记忆)。")
    if ups:
        parts.append("上游成果(在此基础上继续,不要重做、不要复述):\n" + "\n\n".join(ups))
    parts.append(
        "要求:\n1. " + declaration_rule() + "\n"
...
def integration_prompt(plan: Plan, outputs: dict[str, str], budget: int = 14000) -> str:
    ...
    for t in plan.tasks:
        ...
        blocks.append(f"[{t.id} · {t.owner}:{t.title}](状态:{t.status})\n" + (_clip(body, per) if body else "(无产出)"))
    ...
    return (
        ...
        + "最后给出 1~3 条下一步建议或需要用户确认的问题。不要再 @ 任何人。"
    )


def summarize(plan: Plan) -> str:
    """任务板消息的正文(纯文本,给不渲染卡片的地方用)。"""
    lines = [f"分工:{plan.goal}" if plan.goal else "分工"]
    for t in plan.tasks:
