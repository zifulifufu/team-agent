...
def test_add_and_remove_models_and_providers(client):
    ...
    assert c["id"] and "/" not in c["id"]  # non-ASCII names fall back to a random id, which must not contain '/'
...
def test_send_message_streams_over_websocket(client):
    ...
    with client.websocket_connect(f"ws://127.0.0.1/ws/groups/{gid}") as ws:
        ...
        seen = []
        for _ in range(500):                      # bounded: on error the backend emits message_discard / idle, which
                                                  # still terminates the loop instead of waiting forever
            ev = ws.receive_json()
            seen.append(ev["type"])
...
def test_skills_tools_mcp(client):
    # built-in skills come back in English by default; a zh UI returns the same skills with Chinese names
...
def test_token_required_when_configured(tmp_path):
    ...
    with TestClient(app, base_url="http://127.0.0.1") as c:
        assert c.get("/api/health").status_code == 200  # health checks stay open, no token needed
        ...
        # preflight must pass without a token, otherwise the browser/renderer cannot
        # issue requests carrying a custom header
...
def test_dev_mode_blocks_foreign_origins_and_hosts(tmp_path):
    ...
    with TestClient(app, base_url="http://127.0.0.1") as c:
        ...
        # DNS rebinding: Host header is not this machine
