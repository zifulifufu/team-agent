...
PLAN = json.dumps({
    ...
    "tasks": [
        {"id": "t1", "owner": "Copywriter", "title": "初稿", "instruction": "写通知初稿", "strengths": ["writing"], "deliverable": "Markdown"},
        {"id": "t2", "owner": "Proofreader", "title": "审校", "instruction": "审校初稿", "needs": ["t1"], "strengths": ["chinese"]},
    ]}, ensure_ascii=False)
...
async def test_planning_instruction_lists_members_strengths_and_past_actions(store, make_router):
    ...
    orch, g = setup(store, make_router, fake)
    store.add_memory("任务「写发布会通知」→ Copywriter(deepseek-flash);Proofreader(deepseek-flash);共 9 秒", "group", g["id"], "action", "auto")
    c = Collector()
...
async def test_tool_loop_runs_plugin_and_hides_call_markup(store, make_router):
    ...
    c = Collector()
    await orch.handle_user_message(g["id"], "@Copywriter 把 hello 变大写", c)
    msg = c.ends()[0]
...
async def test_memory_injected_scoped_and_toggleable_and_saved_by_tool(store, make_router):
    ...
    c = Collector()
    await orch3.handle_user_message(g["id"], "@Copywriter x", c)
...
async def test_auto_extract_runs_in_background_after_reply(store, make_router):
    ...
    await orch.handle_user_message(g["id"], "@Copywriter 给个结论", Collector())
    await orch.drain()
...
async def test_mcp_tools_are_called_through_real_stdio_server(store, make_router):
    ...
    try:
        c = Collector()
        await orch.handle_user_message(g["id"], "@Copywriter 算 2+3", c)
        msg = c.ends()[0]
