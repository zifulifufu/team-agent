...
LEVELS: dict[str, dict] = {
    "read": {"label": "只读", "desc": "只能读文件、搜索。不改文件、不执行命令、不上网(推荐)。"},
    "edit": {"label": "可改文件", "desc": "在工作目录里读写文件;仍不执行命令、不上网。"},
...
DEFAULT_CFG: dict[str, Any] = {
    ...
    "use_workbuddy_config": False,   # 实验:借用 WorkBuddy 自己的配置目录(为了复用它的登录状态)
...
def clean_cfg(raw: Any, base: dict | None = None) -> dict:
    ...
    if out["level"] == "full":
        ...
        out["risk_ack"] = False
    for k in ("web", "handoff", "use_workbuddy_config"):
        if k in raw:
...
def build_env(cfg: dict, launcher: Launcher | None = None) -> dict[str, str]:
    ...
    env["TERM"] = "dumb"
    if cfg.get("use_workbuddy_config"):
        env["CODEBUDDY_CONFIG_DIR"] = str(Path.home() / ".workbuddy")
...
class StreamParser:
    ...
    def __init__(self, on_delta: DeltaFn | None, on_tool: ToolFn | None):
        ...
        self._streamed = False               # 当前这条助手消息已经靠增量事件输出过
    ...
    async def _user(self, ev: dict) -> None:
        ...
        for b in content:
            if isinstance(b, dict) and b.get("type") == "tool_result":
                ...
                c = b.get("content")
                entry["preview"] = (c if isinstance(c, str) else _block_text(c))[:300]
                await self._tool(idx, entry)
    ...
    def _result(self, ev: dict) -> None:
        ...
        if isinstance(den, list):
            r.denials = [str(d.get("tool_name") or d.get("tool") or d) if isinstance(d, dict) else str(d) for d in den][:10]
